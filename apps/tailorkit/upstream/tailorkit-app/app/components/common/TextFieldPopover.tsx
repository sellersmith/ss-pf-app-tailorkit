...
export default function TextFieldPopover(props: ITextFieldPopoverProps) {
  const { value, setValue, label, labelHidden, activator, maxLength, onBlur: onBlurProp } = props
  ...
  const togglePopoverActive = useCallback(() => {
    ...
    // If the popover is closed and the value has changed, set the value
    if (!popoverActive && tempName !== value) {
      setValue(tempName)
    }

    setPopoverActive(popoverActive => !popoverActive)
  }, [popoverActive, setValue, value])
  ...
  return (
    <Popover active={popoverActive} activator={_activator} onClose={togglePopoverActive} preferredAlignment="left">
      <Popover.Section>
          ...
          <BlockStack gap={'200'}>
            <div ref={inputRef}>
              <TextField
                ...
                {...(!value?.trim().length ? { error: t('name-can-not-empty') } : {})}

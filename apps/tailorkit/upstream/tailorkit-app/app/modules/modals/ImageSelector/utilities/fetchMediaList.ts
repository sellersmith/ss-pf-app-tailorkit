...
const fetchMediaList = async (params: { pageInfo: IPageInfo; isFetchNextPage?: boolean; queryValue?: string }) => {
  ...
  try {
    ...
    const res = await authenticatedFetch(`/api/files?action=${FILE_ACTIONS.FETCH_MEDIA_LISTS}`, {
      method: 'POST',
      body: formData,
    })
    ...
    if (res && res.success) {
      return {
        mediaList: (res.mediaList?.nodes || []).filter((file: any) => file?.fileErrors?.length < 1),

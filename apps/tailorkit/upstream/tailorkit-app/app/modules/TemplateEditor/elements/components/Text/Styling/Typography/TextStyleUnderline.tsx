...
export const TextStyleUnderline = () => {
  ...
  const isActive = textStyle.includes('underline')
  const icon = TEXT_STYLE_OPTIONS.find(opt => opt.value === 'underline')?.label

  const onToggle = useCallback(() => {
    if (!layerStore) return
    const next = isActive ? textStyle.filter(s => s !== 'underline') : [...textStyle, 'underline']
    layerStore.dispatch({
      type: 'UPDATE_LAYER',

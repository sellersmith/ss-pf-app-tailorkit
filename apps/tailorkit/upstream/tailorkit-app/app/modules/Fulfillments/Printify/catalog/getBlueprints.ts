...
export type GetBlueprintsFunc = () => Promise<Blueprint[]>
 ...
 */
const getBlueprints
  = (fetchData: FetchDataFunc): GetBlueprintsFunc =>
  async (): Promise<Blueprint[]> => {
    const response = await fetchData('/catalog/blueprints.json', { method: 'GET' })
    return response
  }

export default getBlueprints

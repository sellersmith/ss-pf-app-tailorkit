...
export type GetListOrdersFunc = (options?: {
  page?: number
  limit?: number
  status?: string
  sku?: string
 ...
 */
const getListOrders
  = (fetchData: FetchDataFunc, shopId: string): GetListOrdersFunc =>
  async (options = {}): Promise<ListOrdersResponse> => {
    ...
    return fetchData(`/shops/${shopId}/orders.json${queryParams ? `?${queryParams}` : ''}`, { method: 'GET' })
  }

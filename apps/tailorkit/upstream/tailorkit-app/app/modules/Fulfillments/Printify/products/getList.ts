...
export type GetListProductsFunc = (options?: { page?: number; limit?: number }) => Promise<ListProductsResponse>
 ...
 */
const getList
  = (fetchData: FetchDataFunc, shopId: string): GetListProductsFunc =>
  async (options = {}): Promise<ListProductsResponse> => {
    ...
    const response = await fetchData(`/shops/${shopId}/products.json${query}`, {
      method: 'GET',
    })
    return response
  }

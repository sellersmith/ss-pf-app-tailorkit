...
export type GetProvidersFunc = () => Promise<Provider[]>
 ...
 */
const getProviders
  = (fetchData: FetchDataFunc): GetProvidersFunc =>
  async (): Promise<Provider[]> => {
    const response = await fetchData('/catalog/print_providers.json', { method: 'GET' })
    return response
  }

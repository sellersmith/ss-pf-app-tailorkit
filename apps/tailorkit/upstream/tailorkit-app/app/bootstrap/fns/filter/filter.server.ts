...
import { supportedOperators, supportedTypes } from '~/bootstrap/fns/filter/constants'
import { getFilterParams, prepareFilter } from './filter.fns'
...
export function parseFilterValue(param: string): FilterValue {
  const filter = prepareFilter(param)

  let { value } = filter
  const { field, type, operator, percentOfResult } = filter
...
export async function getFiltersFromRequest(
  ...
  const listFilters = filterParams.filter(filter => !filter.includes('shopDomain'))
  let _listFilters = excludeShopDomain ? listFilters : [`shopDomain__${shop}`, ...listFilters]

  // Exclude specified filter fields
  if (excludeFilterFields.length) {
    _listFilters = _listFilters.filter(filter => !excludeFilterFields.includes(filter.split('__')[0]))
  }

  // Get list filters
  return parseFilterValues(_listFilters)
}
...
export async function parseFiltersFromRequest(
  ...
  // Get filter values
  const filters = await getFiltersFromRequest(request, excludeShopDomain, excludeFilterFields, shopDomain)
  ...
  for (const filter of filters) {
    ...
    if (field && type && operator && value) {
      ...
      switch (operator) {
        case 'eq':
          if (type === 'date') {
            ...
            if (_value === 'Untitled') {
              ...
              pipeline.push({ $match: { [field as string]: { $regex: `^${_value}$`, $options: 'i' } } })
            }
          } else {
        ...
        case 'has':
          pipeline.push({ $match: { [field as string]: { $regex: _value as string, $options: 'i' } } })
          break

        case 'nh':
          pipeline.push({ $match: { [field as string]: { $not: { $regex: _value as string, $options: 'i' } } } })
          break
        ...
        case 'any':
          if (type === 'array') {
            ...
            pipeline.push({
              $match: {
                [field as string]: { $regex: `^(${(_value as string).split(',').join('|')})$`, $options: 'i' },
              },
            })
          }
          break

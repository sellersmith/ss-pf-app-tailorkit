...
export interface Store<T, A> {
  ...
  getHistory: () => Array<{ action: A; fromState: T; toState: T; diff: any; stackTrace: string[] }>
}

// Global variable for tracing control
let tracingEnabled = true // Enabled by default
let tracingMaxEntries = 500 // Maximum number of trace entries
...
export const createStore = <T extends State, A>(reducer: Reducer<T, A>, initialState: T): Store<T, A> => {
  ...
  //  # ---------------------------------------------------------------#
  const dispatch = (action: A, notifyListeners: boolean = true, skipTrace: boolean = false) => {
  ...
  /** @destroy Completely destroy store: reset state, clear listeners, clear history */
  const destroy = () => {
    state = initialState
    listeners.clear()
    // Clear history for this store
    if (globalStoreHistoryRaw.stores[storeKey]) {
      globalStoreHistoryRaw.stores[storeKey] = []
    }
  }
  ...
  const trace = (action: A, newState: T, stackTrace: string[]) => {
    if (tracingEnabled) {
      ...
      // Check if we've reached the maximum number of trace entries
      if (globalStoreHistoryRaw.histories.length >= tracingMaxEntries) {
        // Remove the oldest entry (shift) before adding a new one
        globalStoreHistoryRaw.histories.shift()
      }

      // Check if store-specific history has reached its limit
      if (globalStoreHistoryRaw.stores[storeKey].length >= tracingMaxEntries) {
        // Remove the oldest entry before adding a new one
        globalStoreHistoryRaw.stores[storeKey].shift()
      }
  ...
  // Enable tracing instance store - simplified
  ...
  return { getState, dispatch, subscribe, resetState, destroy, enableTracing, getHistory }
}

export const useStore = <T extends State, A, U>(store: Store<T, A>, selector: Selector<T, U>): U => {
  // Create a memoized selector that only updates when the selected value changes
  const subscribe = useCallback(
    (onStoreChange: () => void) => {
      let currentSelectedValue = selector(store.getState())

      return store.subscribe(state => {
        const newSelectedValue = selector(state)
        if (!Object.is(currentSelectedValue, newSelectedValue)) {
...
if (typeof window !== 'undefined') {
  ...
  // Add a configuration option for max trace entries
  Object.defineProperty(window, 'tracingMaxEntries', {
    get() {
      return tracingMaxEntries
    },
    set(value) {
      if (typeof value === 'number' && value > 0 && Number.isInteger(value)) {
        tracingMaxEntries = value
      }
    },
  })

  // Helper function to clear existing traces
  window.clearTraces = function () {
    globalStoreHistoryRaw.histories = []
    for (const key in globalStoreHistoryRaw.stores) {
      globalStoreHistoryRaw.stores[key] = []
    }
  }
}

// Add the clearTraces function to the Window interface
declare global {
  interface Window {
    globalStoreHistory: GlobalStoreHistory
    tracingEnabled: boolean
    tracingMaxEntries: number
    clearTraces: () => void
  }
}

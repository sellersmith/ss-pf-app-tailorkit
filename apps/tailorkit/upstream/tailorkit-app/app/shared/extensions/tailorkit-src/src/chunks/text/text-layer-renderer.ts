 ...
 * Text Layer Renderer
 *
 * Complete text rendering implementation for the text chunk.
 * This module contains the full addTextLayer logic that was previously
 * in KonvaCanvasManager, now loaded on-demand to reduce core bundle size.
 *
 * @module chunks/text/text-layer-renderer
 */
import Konva from 'konva'
// Import directly from dedicated module to avoid barrel export pulling in registerOptionSetElements
import { fontStorefrontLoader } from '../../assets/utils/font-storefront-loader'
import {
...
} from '../../shared/libraries/konva/text'
import type { EffectConfig } from '../../shared/libraries/konva/effects/types'
import {
...
} from '../../shared/libraries/svg'

/**
 * Props for adding a text layer
 */
...
/**
 * Interface for the canvas manager passed from core bundle
 * We only need access to specific methods, not the full class
 */
 ...
 *
 * This is the full implementation of addTextLayer, moved to the text chunk
 * to reduce core bundle size for products without text layers.
 *
 * @param manager - The canvas manager instance (provides getTargetContainer)
 * @param props - Text layer configuration
 * @returns The created Konva node (Text, TextPath, or Image)
 */
export async function renderTextLayer(
  ...
  const targetContainer = manager.getTargetContainer()
  ...
  // Separate effects by type using shared utility
  const { dropShadows, innerShadows, hasEffects: hasEffectsFromShadows } = separateEffects(derivedEffects)

...
/**
 * Parameters for rendering a leather technique path with SVG
 */
interface LeatherTechniquePathParams {
  pathData: string
...
/**
 * Render a path with native SVG filter rendering
 * This produces identical output to VectorEditor by using the same SVG filter pipeline
 * Works for both leather techniques (debossing, embossing, etc.) and jewelry techniques
 * (diamond-drag, laser-annealing, etc.)
 *
 * IMPORTANT: Some technique filters handle opacity internally via feColorMatrix.
 * We must NOT apply fill-opacity here, as this would:
 * 1. Double-apply opacity (once via fill-opacity, once via filter's feColorMatrix)
 * 2. Affect SourceAlpha used by feOffset/feComposite operations in the filter chain
 *
 * The filter's internal opacity calculation expects SourceGraphic alpha = 1.0
 */
async function renderPathWithSvgFilter(
  ctx: CanvasRenderingContext2D,
  params: LeatherTechniquePathParams
): Promise<void> {
  const { pathData, filterElement, fill, fillRule, stroke, strokeWidth, scaleX, scaleY, canvasWidth, canvasHeight } =
    params

  // Clone the filter element and serialize it
  ...
  const filterString = new XMLSerializer().serializeToString(filterClone)

  // Create an SVG element with the path and filter
  // Scale the viewBox to match canvas dimensions
  const svgWidth = canvasWidth / scaleX
  const svgHeight = canvasHeight / scaleY

  // Build path attributes - include both fill and stroke if present
  // Note: We intentionally do NOT set fill-opacity/stroke-opacity here because:
  // 1. Leather technique filters handle opacity internally via feColorMatrix
  // 2. Setting opacity would affect SourceAlpha used by the filter chain
  // 3. This matches how VectorEditor renders paths with these filters
 ...
 * @param canvasWidth - Canvas width for SVG rendering
 * @param canvasHeight - Canvas height for SVG rendering
 */
async function renderOverlayPaths(
  ...
  const paths = overlayPaths.querySelectorAll('path')
  ...
  for (const pathEl of paths) {
    ...
    const cssFilter = filterId ? pathFilterCssMap.get(filterId) : null
    ...
    const fillRule = (pathEl.getAttribute('fill-rule') as CanvasFillRule) || 'nonzero'
    const avgScale = Math.sqrt(scaleX * scaleY)

    ctx.save()
    ctx.globalCompositeOperation = blendMode

    // ALWAYS use native SVG filter rendering when a filter element is available
    // This ensures all path filter presets (leather, jewelry, or any future presets)
    // render with full SVG filter primitives for accurate visual output
    if (filterDef?.filterElement && (hasFill || hasStroke) && canvasWidth > 0 && canvasHeight > 0) {
      try {
        await renderPathWithSvgFilter(ctx, {
          pathData: d,
          filterElement: filterDef.filterElement,
          fill,
          fillRule,
          stroke,
          strokeWidth,
        ...
        // SVG rendering handles both fill and stroke, so skip the normal stroke rendering below
        ctx.restore()
        continue
      } catch {
        // Fallback to Canvas 2D for leather techniques only (they have computed fill colors)
        // Other techniques fall through to use CSS filter approximation
        if (isLeatherTechnique) {
        ...
        // For other techniques, fall through to CSS filter rendering below
      }
    }

    // CSS filter fallback when native SVG rendering is not available or fails
    // This provides a basic approximation using CSS filters
    if (cssFilter && !filterDef?.filterElement) {
      ctx.filter = cssFilter
    }

    // Fill the path
    if (fill && fill !== 'none') {
      // For leather techniques without SVG rendering, use Canvas 2D fallback
      if (isLeatherTechnique && filterDef?.presetId) {
        ...
        // Non-leather technique - use original fill
        ctx.globalAlpha = opacity * fillOpacity
        ctx.fillStyle = fill
        ctx.fill(scaledPath, fillRule)
      }
    ...
    if (stroke && stroke !== 'none') {
      ...
      // For leather techniques, also apply the technique color to stroke if present
      if (isLeatherTechnique && filterDef?.presetId) {
        const leatherFillColor = computeLeatherTechniqueFillColor(
          filterDef.presetId,
          filterDef.presetParams || undefined
        ...
        if (leatherFillColor) {
          ctx.strokeStyle = leatherFillColor
        } else {
      ...
      // Scale the stroke width to match the path scaling (use average scale for uniform appearance)
...
export async function compositeImageWithOverlay(options: OverlayCompositorOptions): Promise<CompositorResult> {
  ...
  if (overlayPaths) {
    if (mask) {
      // Render overlay paths to a temporary canvas
...
export function createCachedCompositor() {
  ...
  return {
    ...
    async composite(options: OverlayCompositorOptions): Promise<CompositorResult> {
      const cacheKey = `${options.imageUrl}|${options.overlay.combinedSvg}|${options.targetWidth}|${options.targetHeight}|${options.devicePixelRatio}`

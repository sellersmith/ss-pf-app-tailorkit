...
/**
 * Preset resolver function type for building primitives from preset metadata
 */
export type PresetResolver = (presetId: string, params?: Record<string, number>) => FilterPrimitive[] | null
 ...
 * @param filterElement - The filter XML element string
 * @param presetResolver - Optional function to resolve filter presets by ID
 */
function parseFilter(filterElement: string, presetResolver?: PresetResolver): FilterDef | null {
  // Extract id
  ...
  // If this filter has a preset and a resolver is provided, rebuild primitives from preset params
  // This ensures the filter renders correctly with dynamic primitives
  let finalPrimitives = primitives
  if (presetId && presetResolver) {
    const resolvedPrimitives = presetResolver(presetId, presetParams)
    if (resolvedPrimitives) {
      finalPrimitives = resolvedPrimitives
    }
  }
 ...
 * @param presetResolver - Optional function to resolve filter presets by ID
 */
export function extractFilters(svgString: string, presetResolver?: PresetResolver): Map<string, FilterDef> {
  ...
  while ((match = filterRegex.exec(svgString)) !== null) {
    const filter = parseFilter(match[0], presetResolver)
    if (filter) {

...
import type { CartLineItem, ShopifyCart } from '../types/shopify-cart'
...
export class CartHiddenProductManager {
  ...
  public processCartWithData(cart: ShopifyCart): void {
    ...
    cartItemElements.forEach((el, idx) => {
      ...
      const isHidden = properties && properties[`${PROPERTY_PREFIX}_hidden`] === 'true'
...
/**
 * Helper function to create configuration for specific themes
 */
export function createThemeConfig(themeName: string): Partial<CartHiddenProductConfig> {
  switch (themeName.toLowerCase()) {
    case 'dawn':
      return { cartItemSelector: '.cart-item' }
    case 'debut':
      return { cartItemSelector: '.cart__item' }
    case 'brooklyn':
      return { cartItemSelector: '.cart-item' }
    default:
